...
const Table = (props) => {
	...
	const [state, dispatch] = useState({ min: 0, max: 0, show: 0 })
	const { min, max, show } = state
		...
		<Flex direction="column" gap={6}>
			...
			<TableContainer>
				<ChakraTable>
					...
					<Tbody>{fetched && results.slice(min, max).map((data, index) => td && td(data, index))}</Tbody>
				</ChakraTable>
			</TableContainer>
			...
			{fetched && results.length === 0 && (
				<Flex justify="center" align="center" direction="column" gap={3} px={6} py={24} color="accent-5">
			...
			<Flex display={xsettings.controls === 'off' ? 'none' : 'flex'} justify="space-between" align="center" gap={6}>
				...
				<Flex align="center" gap={3}>
					...
					<IconButton variant="filled" size="lg" color="accent-1" icon={<FiChevronRight size={20} />} disabled={!fetched || (results && results.length <= max)} onClick={next} />
				</Flex>
			</Flex>
		</Flex>
	)
